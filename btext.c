...
int btext_pixelAt(bfont *f, char c, Uint32 x, Uint32 y) {
	...
	if(uc < ' ' || uc > '\x7f')
		return(-1);
	if(x > f->widths[(unsigned int)uc - 32])
		return(-1);
	if(y > f->height)
		return(-1);

	return(f->fontData[(unsigned int)uc - 32][y * f->widths[(unsigned int)uc - 32] + x]);
}
...
void btext_charPoses(bfont *f, const char *text, Uint32 *pos) {
	...
	for(i = 0; utext[i] != '\0'; i++) {
		pos[i] = accum;
		if(utext[i] >= ' ' && utext[i] <= '\x7f')
			accum += f->widths[utext[i] - 32];
		else
			accum += f->widths[95];
	}
}
